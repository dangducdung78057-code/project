...
export const FLAG_META: Record<FeatureFlag, { label: string; desc: string; wired: boolean }> = {
  ...
  merchantGateway: { label: "商家网关与定制派单", desc: "一键比价 / 一键直达 / 二手互转按钮与商家撮合、RFQ 工艺单派发。模块已实现但默认隐藏，接入真实商家后开放。", wired: true },
  visualSearch: { label: "以图搜图（视觉检索）", desc: "上传服装截图检索视觉最相似的商家现货 SKU。引擎已实现（向量检索 + 嵌入适配器预留），默认隐藏，接入 CLIP 推理服务与商家图库后开放。", wired: true },
};
