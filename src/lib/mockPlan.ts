...
import { buildLogisticsTimeline } from "./logisticsCountdown";
...
export function generateMockPlan(input: StageInputData): {
  ...
  // 后勤倒排:经 LogisticsCountdownEngine 统一处理
  // (教学词汇拦截 + >30 人物流缓冲 + 高频动作类安全核验节点 + 日期倒排)
  const reverseSchedule: ScheduleItem[] = buildLogisticsTimeline({
    performanceDate: input.performanceDate,
    weeklyFreq: input.rehearsalFrequencyPerWeek,
    peopleCount: total,
    programType: input.programType,
    baseMilestones: [
      { daysBefore: 30, category: "排产", task: "确认服装总表与预算(本表)", owner: "排产负责人" },
      { daysBefore: 25, category: "采购", task: "供应商比价与样衣下单", owner: "采购" },
      { daysBefore: 18, category: "校验", task: "样衣验收 + 尺码摸底", owner: "排产 + 班主任" },
      { daysBefore: 14, category: "采购", task: "主料全量下单", owner: "采购" },
      { daysBefore: 7, category: "物流", task: "到货验收 + 试穿窗口", owner: "排产" },
      { daysBefore: 3, category: "现场", task: "合装检查 + 备件到位", owner: "现场" },
      { daysBefore: 1, category: "现场", task: "终检 + 应急包封装", owner: "现场" },
    ],
  });
