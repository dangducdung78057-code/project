...
import { ChevronDown, ChevronRight, ExternalLink, ShoppingBag, Scale, Zap, Recycle } from "lucide-react";
...
import { priceCompare, directReach, secondHandSearch, isGatewayConnected } from "@/lib/merchantGateway";
    ...
    <>
      ...
      {open && (
        <div className="mt-2 space-y-2 rounded-md border border-dashed border-border bg-muted/30 p-2">
          ...
          {/* 商家网关操作(模块二):默认隐藏,接入真实商家后经 merchantGateway 开关开放 */}
          {flags.merchantGateway && <GatewayActionsBar item={item} />}
          {candidates.map((c, i) => (
...
/** 商家网关预留操作栏:未接入商家时点击展示预留提示,接入后自动切换为真实链路 */
function GatewayActionsBar({ item }: { item: PlanItem }) {
  const [notice, setNotice] = useState<string | null>(null);
  const connected = isGatewayConnected();

  const run = async (action: "compare" | "direct" | "secondhand") => {
    const r =
      action === "compare"
        ? await priceCompare(item, 1)
        : action === "direct"
          ? await directReach("", item, 1)
          : await secondHandSearch({ category: item.category });
    if (r.reserved) setNotice(r.message);
    else setNotice(null);
  };
  ...
  return (
    <div className="space-y-1.5">
      <div className="flex flex-wrap items-center gap-1.5">
        <Button type="button" size="sm" variant="outline" className="h-6 gap-1 px-2 text-[11px]" onClick={() => run("compare")}>
          <Scale className="h-3 w-3" />
          一键比价
        </Button>
        <Button type="button" size="sm" variant="outline" className="h-6 gap-1 px-2 text-[11px]" onClick={() => run("direct")}>
          <Zap className="h-3 w-3" />
          一键直达
        </Button>
        <Button type="button" size="sm" variant="outline" className="h-6 gap-1 px-2 text-[11px]" onClick={() => run("secondhand")}>
          <Recycle className="h-3 w-3" />
          二手互转
        </Button>
        {!connected && <ToneBadge tone="info">接口预留中</ToneBadge>}
      </div>
      {notice && (
        <div className="rounded border border-border bg-muted/50 px-2 py-1.5 text-[11px] text-muted-foreground">{notice}</div>
      )}
    </div>
  );
}
